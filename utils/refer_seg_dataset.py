...
class ReferSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.clip_image_processor = CLIPImageProcessor.from_pretrained(vision_tower) if type(vision_tower) is str else vision_tower 
    ...
    def __getitem__(self, idx):
        ...
        # image_clip: torch.Size([3, 224, 224])
        # masks: torch.Size([3, 424, 640])
        boxes = []
        for m in masks:
            np_mask = np.array(m)
            if (np_mask > 0).any():
                y1, y2, x1, x2 = self.bounding_box(np_mask)
        ...
        boxes[:, 0::2].clamp_(min=0, max=masks.shape[2])
        boxes[:, 1::2].clamp_(min=0, max=masks.shape[1])
        
        original_shapes = [m.shape[:2] for m in masks]
        resized_shapes = [self.clip_image_processor.resize(orig_image, self.clip_image_processor.size, self.clip_image_processor.resample).shape[:2] for m in masks]
        target_size = tuple(self.clip_image_processor.crop_size.values())

        trajectoies = box_clip_transform(boxes, original_shapes, resized_shapes, target_size)
