...
from .utils import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                    DEFAULT_IMAGE_TOKEN, DEFAULT_VD_END_TOKEN, DEFAULT_VD_START_TOKEN)
from .vqa_dataset import VQADataset


def collate_fn(
    batch, tokenizer=None, conv_type="llava_v1", use_mm_start_end=True, local_rank=-1, wrap_as_video = False
):
    image_path_list = []
    images_list = []
    images_clip_list = []
    ...
    if use_mm_start_end:
        ...
        for i in range(len(conversation_list)):
            if not wrap_as_video:
                ...
                conversation_list[i] = conversation_list[i].replace(DEFAULT_IMAGE_TOKEN, replace_token_vid)
                conversation_list[i] = conversation_list[i].replace(DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN * 1)
                conversation_list[i] = conversation_list[i].replace(DEFAULT_IMAGE_TOKEN, replace_token_img)
