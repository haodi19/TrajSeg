...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sampled_sents = np.vectorize(sents.__getitem__)(sampled_inds).tolist()  # mq select 3 sents
        sampled_masks = [
        ...
        if self.explanatory != -1 and image_name in self.img_to_explanation:
            if random.random() < self.explanatory:  # mq 10% to only answer and do not segment
                choice = 2
            else:
        ...
        for text in sampled_sents:
            ...
            i = 0
            while i < len(questions):   # mq TODO: only run at last
                conv.messages = []
