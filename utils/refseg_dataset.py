...
from datasets.refexp2seq import build as build_seq_refexp
...
class RefSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.refseg_datas = refseg_data.split("||")
        # self.train_mode_rate = train_mode_rate
        self.train_mode = 0
        
        # self.refseg_datas = ['refcoco', 'refcoco+', 'refcocog']
        for name in self.refseg_datas:
            self.data2list[name] = build_seq_refexp(name, 'train', os.path.join(base_video_dir, 'coco'), args)
    ...
    def __getitem__(self, idx):
        ds = random.randint(0, len(self.refseg_datas) - 1)
        ds = self.refseg_datas[ds]
        
        unvalid_sample = True
        while unvalid_sample:
            if ds in ["refcoco", "refcoco+", "refcocog"]:
                refseg_data = self.data2list[ds]
                # image, labels = self.data2list[ds]
                idx = random.randint(0, len(refseg_data.ids) - 1)
                imgs, target = refseg_data[idx]
