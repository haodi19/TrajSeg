...
def parse_args(args):
	...
	# YTVOS
	parser.add_argument("--revos_path", type=str, required=True)
	...
	parser.add_argument("--llm_sample_frames", type=int, default=10)
...
def get_specific_level_directories(path, level):
    specific_level_dirs = []
    path = os.path.realpath(path)
    target_depth = path.count(os.sep) + level
    for root, dirs, files in os.walk(path):
        depth = root.count(os.sep)
        if depth == target_depth:
            relative_path = os.path.relpath(root, path)
            specific_level_dirs.append(relative_path)
    return specific_level_dirs
...
def main(args):
	...
	# 1. load data
	root = Path(args.revos_path) # data/ref-youtube-vos
	img_folder = os.path.join(root)
	meta_file = os.path.join(root, f"meta_expressions_{split}_.json")
	...
	# done_videos = get_specific_level_directories('/hhd2/ljn/tmp_lisa/final_lisa/Lisa2/test_nn_image_l6_video_4000_revos/Annotations', 3)
...
def sub_processor(pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
	...
	for video in video_list:
		...
		for i in range(num_expressions):
			...
			meta["frames"] = sorted(data[video]["frames"])
			metas.append(meta)
		meta = metas
