...
text_module = ext_loader.load_ext('_ext', [
...
def get_indice_pairs(indices,
    ...
    if grid is None:
        if ndim == 2:
            get_indice_pairs_func = text_module.get_indice_pairs_2d_forward
        elif ndim == 3:
            get_indice_pairs_func = text_module.get_indice_pairs_3d_forward
        elif ndim == 4:
            get_indice_pairs_func = text_module.get_indice_pairs_4d_forward
        else:
        ...
        if ndim == 2:
            get_indice_pairs_func = text_module.get_indice_pairs_2d_backward
        elif ndim == 3:
            get_indice_pairs_func = text_module.get_indice_pairs_3d_backward
        else:
            raise NotImplementedError
...
def indice_conv(features,
    ...
    if filters.dtype == torch.float32 or filters.dtype == torch.half:
        return text_module.indice_conv_forward(features, filters, indice_pairs,
                                              indice_pair_num,
...
def fused_indice_conv(features, filters, bias, indice_pairs, indice_pair_num,
    ...
    if features.dtype == torch.half or filters.dtypes == torch.float32:
        func = text_module.fused_indice_conv_forward
    else:
        raise NotImplementedError
...
def indice_conv_backward(features,
                         ...
                         subm=False):
    if filters.dtype == torch.float32 or filters.dtype == torch.half:
        return text_module.indice_conv_backward(features, filters, out_bp,
...
def indice_maxpool(features, indice_pairs, indice_pair_num, num_activate_out):
    if features.dtype == torch.float32 or features.dtype == torch.half:
        return text_module.indice_maxpool_forward(features, indice_pairs,
                                                 indice_pair_num,
                                                 num_activate_out)
    else:
        raise NotImplementedError
...
def indice_maxpool_backward(features, out_features, out_bp, indice_pairs,
                            indice_pair_num):
    if features.dtype == torch.float32 or features.dtype == torch.half:
        return text_module.indice_maxpool_backward(features, out_features,
                                                  out_bp, indice_pairs,
                                                  indice_pair_num)
    else:
        raise NotImplementedError
