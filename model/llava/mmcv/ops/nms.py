...
from model.llava.mmcv.utils import deprecated_api_warning
...
def nms(boxes, scores, iou_threshold, offset=0, score_threshold=0, max_num=-1):
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = text_module.nms(*indata_list, **indata_dict)
    else:
...
def soft_nms(boxes,
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = text_module.softnms(*indata_list, **indata_dict)
    else:
...
def nms_match(dets, iou_threshold):
    ...
    if dets.shape[0] == 0:
        ...
        matched = text_module.nms_match(*indata_list, **indata_dict)
        if torch.__version__ == 'parrots':
...
def nms_rotated(dets, scores, iou_threshold, labels=None, clockwise=True):
    ...
    if torch.__version__ == 'parrots':
        keep_inds = text_module.nms_rotated(
            dets_wl,
            scores,
            order,
            dets_sorted,
        ...
        keep_inds = text_module.nms_rotated(dets_wl, scores, order, dets_sorted,
