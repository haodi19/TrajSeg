...
from model.llava.mmcv.cnn import NORM_LAYERS
...
class SyncBatchNormFunction(Function):
    ...
    def forward(self, input, running_mean, running_var, weight, bias, momentum,
        ...
        if batch_size > 0:
            text_module.sync_bn_forward_mean(input3d, mean)
        ...
        if batch_size > 0:
            text_module.sync_bn_forward_var(input3d, mean, var)
        ...
        text_module.sync_bn_forward_output(
            input3d,
            mean,
            var,
            weight,
    ...
    def backward(self, grad_output):
        ...
        if batch_size > 0:
            text_module.sync_bn_backward_param(grad_output3d, norm, grad_weight,
                                              grad_bias)
        ...
        if batch_size > 0:
            text_module.sync_bn_backward_data(grad_output3d, weight,
