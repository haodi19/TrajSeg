...
from model.llava.mmdet.core import bbox2result
from model.llava.mmdet.models import BaseDetector
...
class ONNXRuntimeDetector(DeployBaseDetector):
    ...
    def __init__(self, onnx_file, class_names, device_id):
        ...
        ort_custom_op_path = ''
        try:
            from model.llava.mmcv.ops import get_onnxruntime_op_path
            ort_custom_op_path = get_onnxruntime_op_path()
        except (ImportError, ModuleNotFoundError):
            warnings.warn('If input model has custom op from model.llava.mmcv, \
...
class TensorRTDetector(DeployBaseDetector):
    ...
    def __init__(self, engine_file, class_names, device_id, output_names=None):
        ...
        warnings.warn('`output_names` is deprecated and will be removed in '
                      'future releases.')
        from model.llava.mmcv.tensorrt import TRTWraper, load_tensorrt_plugin
        try:
