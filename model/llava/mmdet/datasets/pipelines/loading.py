...
from model.llava.mmdet.core import BitmapMasks, PolygonMasks
...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            ...
            filename = osp.join(results['img_prefix'],
                                f"COCO_train2014_{results['img_info']['filename']}")
            print(filename)
        else:
    ...
    def __call__(self, results):
        ...
        if not keep.any():
            ...
            for key in keys:
                if key in results:
                    if key == 'gt_labels':
                        results[key] = [results[key][iii] for iii in range(len(keep)) if keep[iii]]
                    else:
                        results[key] = results[key][keep]
            return results
