...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.output_bbox_hypernetworks_mlps = nn.ModuleList(
            [
                MLP(transformer_dim, transformer_dim, 4, 3)
                for i in range(self.num_mask_tokens)
            ]
        )
    ...
    def forward(
        ...
        """
        masks, boxes, iou_pred = self.predict_masks(
        ...
        boxes = boxes[:, mask_slice, :]
        iou_pred = iou_pred[:, mask_slice]

        # Prepare output
        return masks, boxes, iou_pred
    ...
    def predict_masks(
        ...
        # sparse_prompt_embeddings: torch.Size([1, 1, 256])
        ...
        # self.iou_token: Embedding(1, 256)
        # self.mask_tokens: Embedding(4, 256)
        # output_tokens: torch.Size([5, 256])
        
        output_tokens = output_tokens.unsqueeze(0).expand(
            sparse_prompt_embeddings.size(0), -1, -1
        )
        # output_tokens: torch.Size([1, 5, 256])
        
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)
        # tokens: torch.Size([1, 6, 256])
        ...
        for i in range(self.num_mask_tokens):
            ...
            # hyper_in_list[0]: torch.Size([1, 32])
                                        
            box = self.output_bbox_hypernetworks_mlps[i](mask_tokens_out[:, i, :]).sigmoid()
            boxes.append(box)
